...
public class JoinQueryInterceptor extends JsqlParserSupport implements InnerInterceptor {
     ...
     * 保存我们的关联查询的上下文信息
     */
    static ThreadLocal<List<JoinBuilder>> JOIN_BUILDER = new ThreadLocal<>();
    ...
    public void beforeQuery(Executor executor, MappedStatement ms, Object parameter, RowBounds rowBounds,
        ...
        if (parameter instanceof MapperMethod.ParamMap) {
            Collection values = ((MapperMethod.ParamMap) parameter).values();
            for (Object value : ((MapperMethod.ParamMap) parameter).values()) {
                if (value instanceof JoinQueryWrapper) {
                    List<JoinBuilder> joinBuilders = ((JoinQueryWrapper) value).getJoinBuilder();
                    if (!CollectionUtils.isEmpty(joinBuilders)) {
                        JOIN_BUILDER.set(joinBuilders);
                        try {
    ...
    protected void processSelect(Select select, int index, Object obj) {
        List<JoinBuilder> joinBuilders = JOIN_BUILDER.get();
